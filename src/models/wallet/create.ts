...
export interface CreateWalletModel {
  ...
  create: Thunk < CreateWalletModel, {
    username: string;
    password: string;
  }
  ...
  setHiddenSeedWord: Thunk < CreateWalletModel, {
    word: string;
    position: number;
  }
  , Injections, StoreModel > ;
...
const createWalletModel: CreateWalletModel = {
  ...
  setGeneratedSeed: action((state, seed) => {
    ...
    let newSeed: {position: number; text: string; disabled: boolean;}[] =
  ...
  setHiddenSeedWord: thunk((actions, payload, {injections, getStoreState}) => {
    const index = payload.position - 1;
    let word = {...getStoreState().createWallet.hiddenSeed[index]};
    word.text = payload.word;
    if (getStoreState().createWallet.generatedSeed[index].text === word.text) {
      word.disabled = true;
    }
    let newSeed: ISeed[] = [];
